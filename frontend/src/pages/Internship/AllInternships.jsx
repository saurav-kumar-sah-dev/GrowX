...
import { Briefcase, Search, MapPin, Clock, DollarSign, ArrowRight, Sparkles, Loader2, Filter, X, Building2, Users, Globe as GlobeIcon, Zap, Award, Star, Calendar, GraduationCap } from 'lucide-react';
...
const COLORS = ["#10b981", "#34d399", "#059669", "#6ee7b7", "#a7f3d0", "#d1fae5", "#059669", "#047857"];
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        const internshipJobs = allJobs.filter(j => j.status === 'active' && j.jobType === 'Internship');
  ...
  return (
    <div className="min-h-screen" style={{ background: C.obsidian }}>
      <style>{`
        ...
        .internships-root { font-family: 'DM Sans', sans-serif; }
        ...
        <div className="relative overflow-hidden py-16 px-4" style={{ 
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              <GraduationCap size={14} color={C.green} />
              <span className="text-xs font-medium" style={{ color: C.green, fontFamily: "'DM Mono', monospace" }}>
                INTERNSHIP OPPORTUNITIES
              </span>
            </div>

            <h1 className="text-4xl md:text-5xl font-bold mb-4" style={{ fontFamily: "'Playfair Display', serif", color: C.ivory }}>
              All <span style={{ color: C.green }}>Internships</span>
            </h1>

            <p className="text-lg" style={{ color: C.ivoryMuted }}>
              {loading ? 'Loading...' : `${filteredJobs.length} internships found`}
            </p>
          </motion.div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row gap-4 mb-8">
            ...
            <button
              ...
              style={{ 
                ...
                color: showFilters ? 'white' : C.ivory,
                border: `1px solid ${C.greenBorder}`
              }}
            >
              <Filter size={20} />
              ...
              {activeFilters > 0 && (
                <span 
                  ...
                  style={{ background: C.obsidian, color: C.green }}
                >
                  {activeFilters}
                </span>
              )}
          ...
          {showFilters && (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-medium mb-3" style={{ color: C.ivoryMuted }}>Work Mode</label>
                  <div className="flex flex-wrap gap-2">
                    {["All", "On-site", "Remote", "Hybrid"].map(type => (
                      <button
                        ...
                        style={{
                          background: jobType === type ? C.green : 'transparent',
                          color: jobType === type ? 'white' : C.ivoryMuted,
                          border: `1px solid ${jobType === type ? C.green : C.greenBorder}`
                        }}
                      >
                        {type}
                      </button>
                ...
                <div>
                  <label className="block text-sm font-medium mb-3" style={{ color: C.ivoryMuted }}>Duration</label>
                  <div className="flex flex-wrap gap-2">
                    {["All", "1 Month", "2-3 Months", "4-6 Months"].map(level => (
                      <button
                        key={level}
                        onClick={() => setExperience(level)}
                        className="px-4 py-2 rounded-xl text-sm font-medium transition-all"
                        style={{
                          background: experience === level ? C.green : 'transparent',
                          color: experience === level ? 'white' : C.ivoryMuted,
                          border: `1px solid ${experience === level ? C.green : C.greenBorder}`
                        }}
                      >
                        {level}
                      </button>
          ...
          {loading ? (
            <div className="flex items-center justify-center py-20">
              <Loader2 className="animate-spin" size={48} color={C.green} />
            </div>
            ...
            <motion.div 
              ...
              style={{ background: C.surface, border: `1px solid ${C.greenBorder}` }}
            >
              <GraduationCap size={64} className="mx-auto mb-4" color={C.ivoryMuted} style={{ opacity: 0.3 }} />
              <p className="text-xl font-bold mb-2" style={{ color: C.ivory }}>No internships found</p>
                  ...
                  >
                        ...
                        <div className="flex items-start justify-between gap-2 mb-1">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="px-2 py-0.5 rounded-full text-xs font-semibold" style={{ background: `${color}20`, color: color }}>
                                Internship
                              </span>
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      ...
                      <div className="flex items-center gap-2 text-sm" style={{ color: C.ivoryMuted }}>
                        ...
                        <span className="truncate" style={{ color: C.success }}>{job.salary || 'Stipend'}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm" style={{ color: C.ivoryMuted }}>
                        <Clock size={14} className="shrink-0" /> 
                        <span className="truncate">{job.duration ? `${job.duration} months` : '3 months'}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm" style={{ color: C.ivoryMuted }}>
                      ...
                      <div className="flex items-center gap-2 text-sm" style={{ color: C.ivoryMuted }}>
                        ...
                        <span className="truncate">{job.stipendType || 'Negotiable'}</span>
                      </div>
                      <div className="flex items-center gap-2 text-sm" style={{ color: C.ivoryMuted }}>
                        <Users size={14} className="shrink-0" /> 
                        <span className="truncate">{job.positionsOpen || 1} Position{job.positionsOpen > 1 ? 's' : ''}</span>
                      </div>
                    </div>
                    ...
                    {job.skills && job.skills.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-4">
                        ...
                        {job.skills.length > 4 && (
                          <span 
                            className="px-3 py-1 rounded-full text-xs font-medium"
                            style={{ background: C.surface, color: C.ivoryMuted, border: `1px solid ${C.greenBorder}` }}
                          >
                    ...
                    <motion.div 
                      ...
                      style={{ borderTop: `1px solid ${C.greenBorder}` }}
                    >
