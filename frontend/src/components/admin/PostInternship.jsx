...
const PostInternship = () => {
  ...
  const [input, setInput] = useState({
    ...
    applicationDeadline: '', whoCanApply: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Error posting internship');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="relative overflow-hidden rounded-3xl p-8 shadow-2xl" style={{ background: `linear-gradient(135deg, ${C.green}, ${C.greenDark})` }}>
            ...
            <div className="relative flex items-center justify-between">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <h1 className="text-4xl font-black text-white mb-2">Create Internship</h1>
                  <p className="text-white/80 text-lg">Post a new internship opportunity</p>
                </div>
              </div>
                ...
                <div className="space-y-6">
                  <div>
                    <label className="block text-sm font-bold text-[#F5F0E6] mb-3 flex items-center gap-2">
                      <GraduationCap className="w-5 h-5" style={{ color: C.gold }} />
                ...
                {input.stipendType !== 'Unpaid' && (
                  <div className="p-6 rounded-3xl" style={{ background: `linear-gradient(135deg, ${C.goldDim}, transparent)`, border: `1px solid ${C.goldBorder}` }}>
                    ...
                    <Input
                      ...
                      className="h-16 px-6 rounded-2xl text-lg font-bold text-[#F5F0E6] placeholder:text-[#A8A099] border-2 border-[#D4A853]"
                      style={{ background: '#0A0A0F' }}
                    />
